<?php
// cron_drug_notifications.php

// Set the timezone
date_default_timezone_set('Africa/Nairobi'); // Change to your timezone

// Database configuration
require_once 'includes/database.php'; // Adjust path as needed
require_once 'classes/Pharmacy.php';  // Adjust path as needed

// Log function
function logMessage($message) {
    $logFile = __DIR__ . '/cron_logs/drug_notifications.log';
...
try {
    logMessage("Starting drug notifications cron job");
    
    // Your generateDrugNotifications function
    function generateDrugNotifications($db) {
        ...
        $deletedCount = $db->getPdo()->rowCount();
        ...
        return [
            ...
            'total_new' => $expiringCount + $lowStockCount
        ];
    }
    ...
    logMessage("Deleted old notifications: " . $result['deleted_old']);
    logMessage("New expiring drug alerts: " . $result['expiring']);
    logMessage("New low stock alerts: " . $result['low_stock']);
    logMessage("Total new alerts: " . $result['total_new']);
